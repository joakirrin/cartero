...
6. Repository & Presentation Layer
7. File Management (Deprioritized)
...
1. Start session with the current session brief (generated by `cartero session` or manually from master file)
...
Currently: session brief is generated manually and pasted at the start of the conversation.
Eventually: `cartero session` generates the brief and feeds it directly to the LLM via API.
